...
\texttt{week10\_evaluation\_summary.json}  & JSON summary of all evaluation results  & 1.7 KB \\
\bottomrule
\end{tabular}
\end{table}
...
It favors content methods by construction; a user-history holdout would be stronger.
...
\begin{itemize}[nosep]
  \item \textbf{Popularity global} (NDCG@10 = 0.8390) is the weakest system: genre-blind,
    high per-query variance, and systematically fails for niche genres.
    It serves as a useful performance floor.
  \item \textbf{Content cosine} (NDCG@10 = \textbf{0.9925}) is the strongest system,
    achieving near-perfect genre-relevant retrieval by leveraging the
    dense autoencoder embedding space from Week~7.
  \item \textbf{SVD collaborative} (NDCG@10 = 0.9496) is the strongest behavioral model,
    capturing co-occurrence patterns that content features miss ---
    particularly valuable for genre-pure titles.
\end{itemize}
